...
(defun asm-start-of-line (asm-reader my-char)
  (cond
    ...
    ((equal my-char "#")
     (push-current-and-set-state "hash-sign-read" asm-reader))
    ((equal my-char "(")
    ...
    ((equal my-char "[")
     (push-current-and-set-state "opening-square-bracket" asm-reader)
     (setf (is-there-code-on-this-line asm-reader) t)
     (setf (ast-string asm-reader) (concatenate 'string (ast-string asm-reader) "'(\"[")))
    ((equal my-char "]")
    ...
    ((equal my-char "/")
     (push-current-and-set-state "slash" asm-reader))
                ...
                (cond
                  ...
                  ((equal (current-state asm-reader) "inside-instruction")
                   (cond
                     ...
                     ((equal my-char ",")
                      (push-current-and-set-state "in-space" asm-reader)
                      (setf (ast-string asm-reader) (concatenate 'string (ast-string asm-reader) "\"")))
                     ;; is character space?
                     ;; if yes, mark we are inside space, output "
                     ((equal my-char " ")
                   ...
                   (cond
                     ...
                     ((equal my-char "e")
                      (return-from transform-code-to-string (concatenate 'string
                                                   ...
                                                   "#a"
                                                   (coerce (list #\Newline) 'string)
                                                   (eval (read-from-string (lisp-code-string asm-reader)))
                                                   (coerce (list #\Newline) 'string)
                                                   "#e"
                                                   (coerce (list #\Newline) 'string)
                                                   "))")))
                     ...
                     (t (setf (lisp-code-string asm-reader) (concatenate 'string (lisp-code-string asm-reader) "#" my-char)))))
                  ;; is character # ?
